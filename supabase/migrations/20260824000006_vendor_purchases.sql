-- Vendor purchases: admin-only creation (RLS already restricts writes on
-- vendors/vendor_purchases/vendor_purchase_items to is_tenant_admin; staff
-- can only read). Since create_vendor_purchase is security definer and
-- bypasses RLS, it must re-check is_tenant_admin itself -- unlike
-- create_invoice/create_quotation/create_job_sheet, which only check
-- is_tenant_member because staff ARE allowed to create those.
--
-- Auto-linking: when purchase_kind = 'stock_in_trade', a 'product' line
-- creates one products row per unit (products are individually serialized,
-- one row per physical device -- quantity must be a whole number), and a
-- 'spare' line creates a single spare_parts row carrying the whole line's
-- quantity (spare_parts is quantity-tracked, one row per part). The
-- resulting ids are written back onto vendor_purchase_items.product_ids /
-- .spare_part_id. Lines with item_type 'other', or any line on an
-- office_expense/capital_asset purchase, are recorded but never touch
-- inventory.

-- ---------------------------------------------------------------------------
-- vendor_purchase_number_counters: one counter per tenant.
-- ---------------------------------------------------------------------------
create table public.vendor_purchase_number_counters (
