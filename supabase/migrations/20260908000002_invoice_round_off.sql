-- Optional "round off total" on invoices. When enabled, the final total is
-- rounded to the nearest whole rupee and the delta (which can be positive
-- or negative, e.g. -0.16) is stored separately in round_off_amount rather
-- than silently absorbed into final_price, so the invoice still reconciles
-- line-by-line.
...
-- ---------------------------------------------------------------------------
-- create_invoice: adding p_round_off is a genuine signature change (a new
-- parameter), which `create or replace function` can't apply in place --
-- Postgres treats a different argument list as a distinct overload, and
-- PostgREST can't reliably resolve which overload a named-arg RPC call
-- means once two exist. Drop the old signature first so there's only ever
-- one create_invoice, same approach as create_job_sheet's device_password
-- addition.
-- ---------------------------------------------------------------------------
...
-- this is a body-only replace. If the source invoice had round_off = true,
-- the round-off is recomputed fresh against the post-conversion total
-- (adding GST changes the total, so the old delta is no longer correct)
-- rather than copying v_source.round_off_amount across.
-- ---------------------------------------------------------------------------
